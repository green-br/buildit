...
class FftwBase(AutotoolsPackage):
    ...
    def autoreconf(self, spec, prefix):
        ...
        if spec.satisfies("@master"):
            which("touch")("ChangeLog")
            which("autoreconf")("--verbose","--install","--symlink","--force")
    ...
    def configure(self, spec, prefix):
        ...
        if spec.satisfies("@master"):
            options.append("--enable-maintainer-mode")
        ...
        for precision in self.selected_precisions:
            ...
            with working_dir(precision, create=True):
                configure(*opts)
                if self.spec.satisfies("@master"):
                    copy('../genfft/*.ml', './genfft')
                    copy('../api/genf03.pl', './api')
                    copy('../api/fftw3.h', './api')
                    copy('../api/genf03.pl', './mpi')
                    copy('../mpi/fftw3-mpi.h', './mpi')
                    copy('../mpi/genf03-wrap.pl', './mpi')
...
class Fftw(FftwBase):
    ...
    depends_on("opam", type="build", when="@master")
    depends_on("opam-ocamlbuild", type="build", when="@master")
    depends_on("opam-num", type="build", when="@master")
    depends_on("opam-ocamlfind", type="build", when="@master")
